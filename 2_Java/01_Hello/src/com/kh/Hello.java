 ...
 *   - 작성된 소스파일을 자바 컴파일러(java.exe)로 컴파일 하여
 ...
 *    서로 관련된 코드들을 그룹으로 나누어 별고의 클래스를 구성한다.
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	 ...
	 * - 해당 메서드 호출시 메서드의 블럭({})에 해당하는 부분을 실행한다.
	 * */
	// 실행 메서드 (main 메서드) : 자바 프로그램 실행 시 가장 먼저 호출
	// 모든 클래스가 main 메서드를 가지고 있어야 하는 건 x
	// 단, 하나의 Java 애플리케이션에는 main 메서드를 포함한 클래스가 반드시 하나는 있어야 한다.
	 ...
	 * - 클래스 : 대문자로 시작, 여러개의 단어가 있을 때는 단어 앞자리 마다 대문자로 표기
	 * - 메서드 : 소문자로 시작, 여러개의 단어가 있을 때는 단어 앞자리 마다 대문자로 표기
	 * */
