 ...
 * - 추상 클래스보다 더 강한 규칙성, 강제성을 가진것이 인터페이스다.
 * 
 * 추상클래스와 인터페이스
 * 공통점
 * - 객체 생성은 안된다. 단, 참조변수로는 사용이 가능한다.(즉, 다형성 적용)
 * - 상속하는 클래스에서 추상 메서드를 재정의(오버라이딩) 하도록 강제한다.
 * 
 * 차이점
 * - 추상클래스는 abstract 키워드로 클래스가 정의되어있고, 클래스내에 변수, 메서드 생성 가능
 * - 인터페이스는 interface 키워드로 정의되어있고, 인터페이스 내에 변수, 메서드 생성 불가능
 * 
 * extends 와 implements
 * - 클래스간의 상속관계일 때 : 자식 클래스명 extends 부모 클래스명
 * - 클래스와 인터페이스의 구현 관계일 때 : 구현 클래스명 implements 인터페이스명, 인터페이스명, ...
 * - 인터페이스간의 상속관계일 때 : 인터페이스명 extends 인터페이스명, 인터페이스명, ... 
 * */
