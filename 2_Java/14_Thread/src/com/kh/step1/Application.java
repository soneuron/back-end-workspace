 ...
 * 	 한 프로세스내에 여러개의 스레드가 존재한다면 여러 개의 실행 흐름이 생긴다는 의미
 * 
 * * 메인 스레드
 * - 모든 자바 프로그램은 메인 스레드가 main()메서드를 실행하면서 시작
 * - main()메서드의 첫 코드부터 아래로 순차적으로 실행되고,
 *   코드를 모두 실행하거나 return 문을 만나면 프로그램이 실행 종료
 *   
 * * 멀티 스레드
 * - 멀티스레드는 하나의 프로세스 내에서 여러 스레드가 동시에 작업을 수행하는 것
 * - 실행 중인 스레드가 하나라도 있다면 프로세스가 종료되지 않는다
 * - 하나의 프로세스 내부에 여러개의 스레드가 생성되기 때문에
 *   하나의 스레드가 예외를 발생시키면 프로세스 자체가 종료 될 수 있다
 *   
 * * 장점
 * - 시스템 자원을 보다 효율적으로 사용
 ...
 * - 교착항태(dead-lock)가 발생하지 않도록 주의
 * - 각 스레드가 효율적으로 고르게 실행 될 수 있게 해야한다
 * */
