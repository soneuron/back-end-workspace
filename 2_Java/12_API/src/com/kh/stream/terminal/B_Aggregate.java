...
public class B_Aggregate {
	...
	/*
	 * 기본 집계
	 * - 최종 처리 기능으로 요소들을 개수(count), 합계(sum), 평균값(avg), 최대값(max), 최소값(min) 등과 같이 하나의 값으로 산출 하는 역할
	 * 		- count : 요소의 개수 리턴
	 * 		- sum : 요소들의 함계 리턴
	 * 		- average : 요소들의 평균 리턴
	 * 		- max : 최대 요소 리턴
	...
	public void method1() {
		 ...
		 * - 단순히 값만 저장하는 것이 아니라, 값의 존재 여부를 확인하고 값이 존재하지 않을 경우 디폴트 값을 설정 할 수 있는 객체
		 * */
		
		OptionalDouble avg = Arrays.stream(array).average();
//		System.out.println(avg);
		
		// get : 저장되어 잇는 값을 얻기 위해 사용
