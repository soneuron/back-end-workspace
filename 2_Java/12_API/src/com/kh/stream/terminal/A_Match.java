 ...
 *  - anyMatch : 최소한 한개의 요소가 매개 값으로 주어진 조건을 만족하는 조사
...
public class A_Match {
	...
	public void method1() {
		...
		System.out.println("짝수만 가지고 있는가? " + result); // true // allMatch = and 연산자와 비슷
		
		result = Arrays.stream(values).anyMatch(value -> value % 3 == 0 );
		System.out.println("3의 배구 값이 하나라도 있는가? " + result); // true // anyMatch = or 연산자와 비슷
		
		Arrays.stream(values).noneMatch(value -> value % 5 == 0);
		System.out.println("모두가 5의 배수가 아닌가? " + result); // true
	}
	...
	public void method2() {
		...
		System.out.println("20살 이상인 학생들이 모두 남자인가? " + result1);
		...
		System.out.println("남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는가? " + result2);
