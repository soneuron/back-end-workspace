 ...
 *   자식 인터페이스들이 상속관계를 이루고 있다.
 *   
 * * Stream API
 * - 스트림은 자바 8부터 추가된 기능으로 컬렉션(배열)의 저장 요소드릉ㄹ 하나씩 참조새허
 *   람다식으로 처리 할 수 있도록 해주는 반복자
 * - 중간 처리(intermediate), 최종처리(terminal) 작업을 수행
 *   - 중간처리 : 반복(peek), 매핑(타입변환), 필터링, 정렬 등
 *   - 최종처리 : 반복(forEach), 카운팅, 평균, 총 합 등의 집계 처리
 * - 구분방법은 리턴 타입으로 알 수 있음
 *   - 리턴 타입 Stream이라면 중간 처리 메서드
...
public class Create {
	...
	public void method1() {
		...
		// 첫 번재 매개 값 ~ 두 번째 매개 값 이전까지의 값을 요소로 가지는 스트림 객체 생성
