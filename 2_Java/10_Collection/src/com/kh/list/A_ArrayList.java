 ...
 * - java.util 패키지에 컬렉셔과 관련된 인터페이스와 클래스들이 포함
 * - 컬렉션을 사용하면 데이터의 추가, 삭제, 정렬들의 저리가 간단하게 해결되어
 ...
 * * List 
 * - 자료들을 순차적으로 늘어놓는 구조
 * - 저장되는 객체를 인덱스로 관리하기 때문에 인덱스로 객체를 검색, 삭제 할 수 있는 기능
 ...
 *  	 	 한 시점에서 하나의 스레드만 사용 할 수 있도록 하는 것을 말한다.
 * */
...
//		list.add(3);
	...
	public void method2() {
		...
		list.add(new Person("이시영", "상도동", 42));
		...
		//	  -> 알아서 크기 줄어들고 뒤에 객체를 앞으로 당겨옴
		list.remove(0);
		System.out.println("삭제 후 사람수 : " + list.size());
		...
		// 9. isEmpty : 컬렉션이 비어있는지 확인
		System.out.println("리스트가 비어있는가? " + list.isEmpty()); // 비어있으면 true, 비어있지 않으면 false
		
		
		// 리스트에저장된 사람들의 평균 연령 출력
		int sum = 0;
		...
		Collections.sort(list); // 나이
		System.out.println(list); // 나이
