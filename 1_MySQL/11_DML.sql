/*
    ...
    1) INSERT INTO 테이블명 VALUES (값, 값, ...);
		- 테이블에 '모든 컬럼'에 대한 값을 INSERT하고자 할 때 사용한다.
	...
	2) INSERT INTO 테이블명(컬럼명, 컬럼명, ...) VALUES (값, 값, ...);
        ...
        - 선택이 안된 컬럼들은 기본적으로 NULL값이 들어간다.
		  (단, NOT NULL 조건이 걸려있는 컬럼은 반드시 값을 입력해야 한다.)
		- 기본값(DEFAULT)이 지정되어 있으면 NULL이 아닌 기본값이 들어간다.
	3) INSERT INTO 테이블명 서브쿼리;
		- VALUES 대신 서브 쿼리 조회한 결과값이 통쨰로 INSERT 한다.
...
-- emp_dept 테이블에 employe에서 부서 코드가 D1인 직원의 사번, 이름, 부서코드, 입사일 추가하고
...
/*
    ...
    - SET 절에서 여러 개의 컬럼을 콤마(,)로 나열해서 값을 동시에 변경 할 수 있다.
...
-- emp_salary 에서 이태림 사원의 급여를 3000000원으로, 보너스를 0.4로 변경
UPDATE emp_salary
SET salary = 3000000,
	bonus = 0.4
WHERE emp_name = '이태림';

-- 모든 사원의 급여를 기존 급여세어 10프로 인상한 금액(기존 급여 * 1.1)으로 변경
UPDATE emp_salary
...
SET emp_id = null
WHERE emp_id = 201;
-- >> emp_id 주요키(primary key) 제약조건 위해 (not null 위배) 
...
*/
-- emp_salary 에서 dept_code가 D5인 직원들을 삭제
DELETE FROM emp_salary
WHERE dept_code = 'D5';

/*
	TRUNCATE
    - 테이블 저체 행을 삭제 할 때 사용하는 구문
    - DELETE 보다 수행 속도가 빠르다.
    - 별도의 조건을 제시 할 수 없고 ROLLBACK이 불가능 하다.
    
    TRUNCATE TEABLE 테이블명;
*/
START transaction;
...
ROLLBACK; -- TRUNCATE는 ROLLBACK불가능
