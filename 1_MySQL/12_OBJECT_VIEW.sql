...
-- '러시아'에서 근무하는 사원들릐 사번, 이름, 부서명 , 급여, 근무국가명 조회
SELECT emp_id, emp_name, dept_title, salary, national_name
FROM employee
	JOIN department ON (dept_code = dept_id)
    JOIN location ON (local_code = location_id)
...
-- 2) 뷰에 포함되지 않은 컬럼 중에 베이스가 되는 컬럼이 NOT NULL 에약조건이 지정된 경우
CREATE OR REPLACE VIEW vw_job
AS SELECT job_name FROM job;

-- job_code가 primary key 임으로, not null 제약조건에 걸림.
INSERT vw_job
VALUES('인턴');
...
AS SELECT emp_id 사번, emp_name 사원명, emp_no 주민번호, salary 급여
   FROM employee;
...
VALUES(300, '홍길동', '000000-0000000', 3000000); -- 흐음... 확인 필요 하니 나중에 다시
...
-- 부서별 급여의 합계, 평균을 조회한 SELEST 문을 vw_groupdept 뷰 정의
CREATE OR REPLACE VIEW vw_groupdept
AS SELECT distinct dept_code, sum(salary) 합계, avg(salary) 평균
   FROM employee
   GROUP BY dept_code;
...
-- DELETE (에러)
DELETE FROM cw_dt_job
WHERE job_code = 'J2';

-- 6. JOIN을 이용해서 여려테이블을 연결한 경우
-- 사원들의 사번, 사원명, 주민번호, 부서명 조회한 SELECT문을 vw_joinemp 뷰 정의
...
VALUES(500, '송준호', '800130-1111111'); -- 얘는 또 가능, 어휴.... view 진짜 지 멋대로네
...
-- 7. VIEW 옵션
-- WITH CHECK OPTION : 서브쿼리에 기술된 조건에 부합하지 않는 값으로 수정시 에러발생
-- 급여가 30000000원 이상인 사원들만 조회한 SELECT문을 vw_emp 뷰 정의
