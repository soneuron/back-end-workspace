/*
    ...
    3. 데이터의 중복을 최소화 할 수 있으며, 사용하기 편리하고, 데이터의 무결성, 트랜잭션 처리 등 
...
/*
    ...
    - 테이블에서 테이터를 조회 할 때 사용하는 SQL문
    - SELECT를 통해서 조회된 결과를 RESULT SET이라고 한다. (즉, 조회된 행들의 집합)
    - 조회하고자 하는 컬럼들은 반드시 FROM 절에 기술 한 테이블에 존재하는 컬럼이어야 한다.
...
/*
	컬럼 산술 연산
    - SELECT 절에 컬럼면 입력 부분에 산술연산자를 사용하여 결과를 조회 할 수 있다.
*/
...
/*
	...
	- 별칭을 부여 할 때 띄어쓰기 혹은 특수문자가 포함 될 경우 반드시 큰따옴표(")로 감싸준다.
*/
SELECT 
	emp_name 직원명, 
    salary "직원 월급", 
...
/*
    ...
    !=, <> : 같지 않다
*/
-- employee 에서 부터 코드(dept_code)가 'D9'인 사원들만 조회 (이때, 모든 컬럼 조회)
SELECT *
FROM employee
WHERE dept_code = 'D9';
...
/*
	BETWEEN AND
    -조건식에서 사용되는 구문
    -'~ 이상 ~ 이하'인 범위에 대한 조건을 제시 할 때 사용되는 연산자
...
/*
	LIKE
    - 비교하고자 하는 컬럼값이 내가 제시한 특정 패턴에 만족되는 경우를 조회
    ...
    '_' : 1글자
    비교대상컬럼 LIKE '_문자' => 비교대상컬럼값의 문자 앞에 무조건 한 글자가 올 경우 조회
    비교대상컬럼 LIKE '__문자' => 비교대상컬럼값의 문자 앞에 무조건 두 글자가 올 경우 조회
    비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값의 문자 앞과 뒤에 무조건 한 글자씩 올 경우 조회
*/
...
*/
-- 보너스를 받지 않는 사원(bonus 값이 null)들의 사번(emp_id), 이름(emp_name), 급여(salary), 보너스(bonus) 조회
...
/*
	IN 
    - 비교대상컬럼값이 내가 제시한 목록 중 일치하는 값이 있는지 
...
/*
	ORDER BY
	- SELECT문 가장 마지막 줄에 작성 뿐만이 아니라 실행순서 또한 마지막에 실행
...
FROM employee
-- ORDER BY bonus ASC; -- 보너스 기준 오름차순 정렬 (null이 맨 앞)
ORDER BY 1 DESC; -- 보너스 기준 내림차순 정렬 (null이 맨 뒤)
...
-- LIMIT 절은 두 개의 값이 있을 수 있다
-- 첫 번재 값은 오프셋(offset, 0부터 시작) 시작 행을 지정
...
FROM employee
ORDER BY salary DESC
LIMIT 10 OFFSET 0; -- LIMIT 0, 10; === LIMIT 10 OFFSET 0;

-- 실습문제 ---
-- 1. 직급코드(job_code)가 J7이거나 J2인 사원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM employee
...
-- 6. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어있는 사원들의
--    사번, 사원명, 급여, 보너스 포함 연봉(별칭부터) 조회 (연봉이 높은 순으로 정렬)
--    보너스 포함 연봉 : (SALARY + SALARY * BONUS) * 12
SELECT (salary +salary*bonus)*12 "보너스 포함 연봉", emp_id, emp_name, salary
FROM employee
