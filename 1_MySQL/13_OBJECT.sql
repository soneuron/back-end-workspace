/*
	인덱스(INDEX)
    - SQL 명령문의 처리속도를 향상시키기 위해 행들의 위치 정보를 가지고 있다.
...
/*
	프로시저(PROCEDURE)
    - SQL문을 저장하여 필요 할 때 마다 다시 입력 할 필요 없이 간단하게 호출해서 사용 가능하게 하는 코드 블록
...
CREATE PROCEDURE dal_all_emp(IN id INT)
BEGIN
	DELETE FROM emp_copy
    WHERE emp_id = id;
END //
...
/*
    ...
    DELIMITER ;
*/
-- 사번을 입력받아 해당 사원의연봉을 계산하고 리턴하는 함수 생성
DELIMITER //
...
BEGIN
    ...
    select salary * 12 as result into result from employee where emp_id = id;
...
-- 함수 삭제
DROP FUNCTION IF EXISTS salary_cals;

/*
	트리거(TRIGGER)
    - 특정 이벤트가 발생 할 때ㅑ 자동으로 실행될 내용을 정의하여 저장
    
    ex)
    - 회원탈퇴시 기존 회원테이블에 데이터 DELETE 후
	  곧 바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리
	- 신고 횟수가 일정 수를 넘었으 떄 해당 회원을 블랙리스트로 처리
...
-- employee 테이블에서 부서 코드가 D6인 직원들의 부서 코드를 D3 으로 변경
UPDATE employee
SET dept_code = 'D6'
WHERE dept_code = 'D3';
