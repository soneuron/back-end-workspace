...
/*
	1. 컬럼 추가 / 수정 / 삭제 / 이름 변경
    1-1 컬럼 추가 (ADD)
    ALTER TABLE 테이블명 ADD 컬럼명 자료형 [DEFAULT 기본값]; -- DEFAULT 값이 있을 때 
*/
-- cname 컬럼 추가
...
ALTER TABLE dept_copy MODIFY dept_id INT; -- 에러발생
...
ALTER TABLE dept_copy DROP COLUMN lname; -- 최소 1개의 컬럼이 존재해야 함으로 삭제 불가능
...
*/
-- dept_copy 테이블에서 lname 컬럼명을 location_name 으로 변경
ALTER TABLE dept_copy RENAME COLUMN lname TO lacation_name;

/*
	2. 제약조건 추가 / 삭제 
    2-1 제약조건 추가 (ADD)
    - ADD [CONSTRAINT 제약조건명] PRIMARY KEY(컬럼명);
	- ADD [CONSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES(컬럼명);
...
ALTER TABLE dept_copy ADD CONSTRAINT dept_copy_PK PRIMARY(dept_id);
...
/*
    ...
    - 나머지 : ALTER TABLE 테이블 명 DROP CONSTRAINT 제약조건명;
*/
DROP TABLE dept_copy;
...
-- 삭제하고 싶다면 자식테이블 먼저 삭제 후 부모테이블 삭제 가능
DROP TABLE member;
