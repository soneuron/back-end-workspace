/*
    ...
    - 객체 (object)/ 스키마(Schema)를 만들고(CREATE), 변경하고(ALTER), 삭제(DELETE)하는 언어
...
/*
    ...
    * 자료형
	...
	4. 불리언
		- BOOLEAN / BOOL : 참(True) 또는 거짓(False) 갑을 저장하는데 사용
	5. 이진 자료형
...
INSERT INTO mem_notnull VALUES(2, 'user02', null, '손정배', '남', null, null, current_date());
-- >> NOT NULL 제약조건에 위배되어 오류 발생
INSERT INTO mem_notnull VALUES(2, 'user01', 'pass01', '손정배', '남', null, null, current_date());
...
/*
    ...
    - 추가/수정 시 기존에 있는 데이터 값 중 중복값이 있을 경우 오류 발생
*/
...
/*
    ...
    ex) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 제품번호, 주문번허, 운송장번호, ...
...
INSERT INTO mem_pri
VALUES (null, 'user02', 'pass01', '박세영', '남', null, null, null);
-- >> 기본키에 null을 담으려고 해서 문제 발생(not null 제약조건 위배)

INSERT INTO mem_pri
VALUES (2, 'user02', 'pass01', '박세영', '남', null, null, null);

SELECT * FROM mem_pri;

-- 복합키 사용 예시 (어떤 회원이 어떤 상품을 찜했는지에 대한 데이터를 보관하는 테이블)
-- >> 복합키 방식으로 지정해서 쓰는 건 가급적으로 추천하지 않음
...
INSERT INTO member(mem_id, mem_pwd, mem_name, grade_id)
VALUES('user01', 'pass01', '최제환', null);
-- >> 외래키 제약조건이 부여되도 기본적으로 nullm gjdyd
...
-- >> 부모 테이블(mem_grade)에서데이터 값을 삭제 할 경우 어떤 문제가 발생하는지
-- 데이터 삭제 : DELETE FROM 테이블명 WHERE 조건;

-- mem_grade 테이블에서 grade_code가 2인 등급 삭제
DELETE FROM mem_grade
WHERE grade_code = 2;
-- >> 자식테이블(member)에 2라는 값을 사용하고 있기 때문에 삭제 x 
...
SELECT * FROM member;
DELETE FROM member WHERE mem_no = 6;

/*
	자식테이블 생성시 외래키 제약조건을 부여 할 때 삭제옵션 지정 가능
    - 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리 할 건지
...
DELETE FROM mem_grade
WHERE grade_code = 1;
-- > 삭제 가능, 단 1을 가져다 쓰고있는 자식 데이터 값이 null로 변경
...
/*
	서브쿼리를 이용한 테이블 생성
    - 컬럼명, 데이터 타입, 값 모두 복사 / 제약조건은 NOT NULL만 가능
...
-- 모든행에 대해서 매번 false이기 때문에 테이블 구조만 복사
